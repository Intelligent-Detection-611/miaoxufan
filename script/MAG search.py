...
def read_excel_column_to_list(file_path, sheet_name, column_index=3):
    try:
        try:
            ...
            print(f"Error opening file as ExcelFile: {e}. Trying to read directly...")
...
def get_single_MAG_information(MAG_ID, MAG_path, WL_GeneID, H2_GeneID,
                                  Pyruvate_GeneID, Acetic_Acid_GeneID, Ethanol_GeneID,
                                  RBO_GeneID, Important_GeneID, Secondary_GeneID):
    # 动态获取 sheet 名称
    xls = pd.ExcelFile(MAG_path)
    MAG_sheet_name = xls.sheet_names[0]  # 获取第一个 sheet 的名称

    # 获得单个MAG的GeneID
    MAG_GeneID = read_excel_column_to_list(MAG_path, MAG_sheet_name, column_index=1)
    # 获得WL路径ID
...
def write_all_information_to_xlsx(total_MAG_path, output_path, WL_GeneID, H2_GeneID,
                                  Pyruvate_GeneID, Acetic_Acid_GeneID, Ethanol_GeneID,
                                  RBO_GeneID, Important_GeneID, Secondary_GeneID):
    ...
    for single_MAG_name in os.listdir(total_MAG_path):
        ...
        # 按 '.' 分割文件名，并取前两部分
        MAG_name_parts = single_MAG_name.split('.')
        MAG_name = ".".join(MAG_name_parts[:2])  # 只取前两部分作为MAG_ID
        data_to_output = get_single_MAG_information(MAG_name, single_MAG_path,
                                                    WL_GeneID, H2_GeneID,
                                  Pyruvate_GeneID, Acetic_Acid_GeneID, Ethanol_GeneID,
                                  RBO_GeneID, Important_GeneID, Secondary_GeneID)
...
if __name__ == '__main__':
    file_path = r'D:\Bin_search\30_sample\KOID.xlsx' # Replace with your file path
    ...
    total_MAG_path = r'D:\Bin_search\30_sample\totalMAG'

    #存储最终结果的xlsx文件地址
    output_path = r'D:\Bin_search\30_sample\result.xlsx'
    write_all_information_to_xlsx(total_MAG_path, output_path, WL_GeneID, H2_GeneID,
                                  Pyruvate_GeneID, Acetic_Acid_GeneID, Ethanol_GeneID,
                                  RBO_GeneID, Important_GeneID, Secondary_GeneID)
